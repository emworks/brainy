...
const sayHello = ({
    chat: { id: chatId }
}) => bot.sendMessage(chatId, 'Что вы хотите изучить?');
...
module.exports = ({ esearch }) => {
    const getCourses = ({ searchByQuery }) => async (msg) => {
        logger.add(msg.text, 'query', 'bot');

        if (!_searchResults[msg.chat.id].q ||
            _searchResults[msg.chat.id].q !== msg.text) {
            _searchResults[msg.chat.id] = await searchByQuery(msg.text);
        }
                ...
                .map(async ({ url }, i) => {
                    ...
                    if (i === OFFSET - 1) {
                        if (_searchResults[msg.chat.id].data.length) {
                            ...
                            await bot.once('callback_query', async ({ message, data }) => await getCourses({ searchByQuery })({ 
