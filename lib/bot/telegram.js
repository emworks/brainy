...
const getCourses = ({ search }) => async ({
    ...
    console.log(2, _searchResults[chatId])
    ...
    if (_searchResults[chatId].data.length) {
        await _searchResults[chatId].data
            .splice(0, OFFSET)
            .map(async ({ url }) => await bot.sendMessage(chatId, url));
        ...
        if (_searchResults[chatId].data.length) {
            ...
            bot.sendMessage(chatId, `Найдено ещё ${_searchResults[chatId].data.length}`, options);
        } else {
